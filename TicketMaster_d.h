...
class TicketMaster {
 ...
 public:
  ...
  int getSeatsSold() {
	  ...
	  ifstream reportFile;
	  reportFile.open("salesReport.dat");
	  streampos current = reportFile.tellg();
	  reportFile.seekg(0, reportFile.end);
	  bool empty = reportFile.tellg();
	  reportFile.seekg(current, reportFile.beg);
	  if (empty == 1) {
		  seatsSold = 0;
	  }
	  else
	  {
		  reportFile >> seatsSold;
	  }
